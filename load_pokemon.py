...
for [id, identifier, species_id, weight, height] in con.cursor().execute('select id, identifier, species_id, weight, height from pokemon'):
	...
	if species_id != None:
		moveQuery = 'select move_names.name from pokemon_moves inner join move_names on pokemon_moves.move_id=move_names.move_id where pokemon_id = "{}" and local_language_id=5;'.format(species_id)
		for [move] in con.cursor().execute(moveQuery):
			if move != None:
				loc['moves'].append(move)
